...
class UtilsTestCase(TimeZoneTestCase):
    def test_localtime_for_timezone(self):
        self.assertEqual(
            localtime_for_timezone(datetime(2008, 6, 25, 18, 0, 0), "America/Denver").strftime("%m/%d/%Y %H:%M:%S"),
            "06/25/2008 12:00:00",
        )
...
class LocalizedDateTimeFieldTestCase(TimeZoneTestCase):
    def test_forms_clean_required(self):
        ...
        self.assertEqual(repr(f.clean("2008-05-30 14:30:00")), "datetime.datetime(2008, 5, 30, 14, 30, tzinfo=<UTC>)")
        self.assertRaises(forms.ValidationError, f.clean, "")
